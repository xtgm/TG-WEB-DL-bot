...
| 本地语法检查 | [第 8 步](#step-8) |
| Workers CLI 部署 | [第 9A 步](#step-9a) |
| Workers 手动复制粘贴部署 | [第 9B 步](#step-9b) |
| Workers + GitHub 部署 | [第 9C 步](#step-9c) |
| Pages + GitHub 部署 | [第 9D 步](#step-9d) |
| Pages Wrangler Direct Upload | [第 9E 步](#step-9e) |
...
| [Cloudflare 部署](#step-9a) | Workers 使用 `worker.js`，Pages Functions 使用 `functions/[[path]].js` 复用同一套逻辑 | Workers CLI、Workers 手动、Workers + GitHub、Pages + GitHub、Pages Direct Upload |
...
## 第 9A 步：Workers CLI 部署

适合已经安装 Wrangler、想在本地命令行部署的用户。这个方式会读取 `wrangler.toml`，入口是主目录 `worker.js`。

### 9A-1. 确认 `wrangler.toml`
...
[vars]
PUBLIC_BASE_URL = "https://你的 workers.dev 或自定义域名"
TURNSTILE_SITE_KEY = "你的 Turnstile Site Key"
ADMIN_CHAT_IDS = ""
```

`binding` 必须叫 `DB`，因为代码里固定使用 `env.DB`。
...
### 9A-3. Workers CLI 部署后检查

打开：

```text
https://tg-dualbot-cloudflare.xxx.workers.dev/health
```
...
说明 Worker 入口正常。下一步继续执行 `第 11 步：设置 Telegram Webhook`。

<a id="step-9b"></a>

## 第 9B 步：Workers 手动复制粘贴部署

适合不想用命令行部署代码、想在 Cloudflare Dashboard 里手动创建 Worker 并粘贴 `worker.js` 的用户。

这个方式仍然需要你手动配置 D1、环境变量、Secrets 和数据库表。只复制代码是不够的。

### 9B-1. 在 Dashboard 创建 D1 数据库
...
6. 创建后进入数据库详情页，打开 `Console` 或 `Query`。
7. 打开项目文件：
...
8. 复制整个 SQL 内容，粘贴到 D1 Console 执行。

执行成功后，D1 会创建 `users`、`message_map`、`inbox_messages`、`spam_keywords`、`settings`、`event_logs`、`rate_events`、`ip_verifications` 等表。
...
8. 复制 `worker.js` 的全部代码。
9. 粘贴覆盖 Cloudflare 在线编辑器里的默认代码。
10. 点击 `Save and deploy`。

### 9B-3. 手动绑定 D1

进入这个 Worker 的 `Settings`，找到 `Bindings`，新增 D1 Database Binding：

| 项目 | 值 |
|---|---|
| Binding name / Variable name | `DB` |
| D1 database | `tg-dualbot-db` |

绑定名必须是 `DB`。

### 9B-4. 手动配置 Variables

进入 Worker 的 `Settings` -> `Variables and Secrets`，添加普通变量：

| 名称 | 示例值 | 说明 |
|---|---|---|
| `PUBLIC_BASE_URL` | `https://tg-dualbot-cloudflare.xxx.workers.dev` | Worker 公开地址，不要带结尾 `/` |
| `TURNSTILE_SITE_KEY` | `0x4AAAAA...` | Turnstile Site Key |
| `ADMIN_CHAT_IDS` | `123456789,987654321` | 可选，也可以部署后在后台设置 |
...
| 名称 | 示例值 | 说明 |
|---|---|---|
| `BOT_TOKEN` | `123456789:示例占位符` | Telegram Bot Token |
| `PANEL_PASSWORD` | `你的后台强密码` | Web 后台登录密码 |
| `PANEL_SECRET` | `一长串随机字符串` | Cookie Session 签名 |
| `TELEGRAM_SECRET_TOKEN` | `一长串随机字符串` | Telegram Webhook 校验密钥 |
| `TURNSTILE_SECRET_KEY` | `Turnstile Secret Key` | Turnstile 服务端校验密钥 |
| `PANEL_USER` | `admin` | 可选，不设置默认 `admin` |
...
返回 `ok` 后，继续设置 Telegram Webhook：

```text
https://tg-dualbot-cloudflare.xxx.workers.dev/telegram/webhook
```

<a id="step-9c"></a>

## 第 9C 步：Workers + GitHub 部署

适合希望代码推送到 GitHub 后自动部署 Workers 的用户。
...
不要把真实 `BOT_TOKEN`、后台密码、Cookie 密钥写进 GitHub。

### 9C-2. 推荐方式：Cloudflare Dashboard 连接 GitHub
...
10. 入口以 `wrangler.toml` 的 `main = "worker.js"` 为准。

### 9C-3. Workers + GitHub 必须配置的 Cloudflare 项目设置

在 Cloudflare 的 Worker 项目设置里配置：

| 类型 | 名称 | 值 |
|---|---|---|
| D1 Binding | `DB` | 选择 `tg-dualbot-db` |
| Variable | `PUBLIC_BASE_URL` | `https://你的 Worker 地址或自定义域名` |
| Variable | `TURNSTILE_SITE_KEY` | Turnstile Site Key |
| Variable 或 Secret | `ADMIN_CHAT_IDS` | `123456789,987654321` |
| Secret | `BOT_TOKEN` | Telegram Bot Token |
| Secret | `PANEL_PASSWORD` | 后台密码 |
| Secret | `PANEL_SECRET` | Cookie 随机密钥 |
| Secret | `TELEGRAM_SECRET_TOKEN` | Webhook 校验密钥 |
| Secret | `TURNSTILE_SECRET_KEY` | Turnstile Secret Key |

D1 表结构仍要先执行 `migrations/0001_initial.sql`。

### 9C-4. 可选方式：GitHub Actions 部署 Workers

如果你更想用 GitHub Actions，可以在 GitHub 仓库手动创建：
    ...
    steps:
      ...
      - name: Patch wrangler config
        ...
        run: |
          ...
          s = s.replace('database_id = "REPLACE_WITH_D1_DATABASE_ID"', `database_id = "${process.env.D1_DATABASE_ID}"`);
          s = s.replace('PUBLIC_BASE_URL = ""', `PUBLIC_BASE_URL = "${process.env.PUBLIC_BASE_URL}"`);
          s = s.replace('TURNSTILE_SITE_KEY = ""', `TURNSTILE_SITE_KEY = "${process.env.TURNSTILE_SITE_KEY}"`);
...
| Secret | `D1_DATABASE_ID` | D1 database_id |
| Variable | `PUBLIC_BASE_URL` | Worker 公开地址 |
| Variable | `TURNSTILE_SITE_KEY` | Turnstile Site Key |

真实 Bot Token 和后台密码仍然建议先在 Cloudflare Worker 的 Secrets 里设置，不要写进 GitHub 仓库文件。

<a id="step-9d"></a>

## 第 9D 步：Pages + GitHub 部署

适合希望用 Cloudflare Pages 连接 GitHub 仓库自动部署的用户。这个项目已经提供 Pages Functions 入口：

```text
functions/[[path]].js
```

这个入口会捕获 Pages 上的所有路径，并调用主目录 `worker.js` 里的 `handleRequest()`，所以 Pages 和 Workers 使用同一套业务逻辑。

### 9D-1. 创建 Pages 项目并连接 GitHub
...
11. Root directory 保持仓库根目录。
12. 保存并部署。

### 9D-2. Pages + GitHub 必须配置的绑定和变量

Pages 项目也需要同样的绑定和环境变量。进入 Pages 项目 `Settings`，分别配置 Production 和 Preview 环境，至少 Production 要完整：

| 名称 | 类型 | Pages 中如何配置 |
|---|---|---|
| `DB` | D1 binding | Settings -> Functions -> D1 database bindings，绑定名必须是 `DB` |
| `PUBLIC_BASE_URL` | Environment variable | Settings -> Environment variables |
| `TURNSTILE_SITE_KEY` | Environment variable | Settings -> Environment variables |
| `ADMIN_CHAT_IDS` | Environment variable 或 Secret | 可在 Pages 环境变量里设置，也可登录后台设置 |
| `BOT_TOKEN` | Secret | Pages 加密变量或 `wrangler pages secret put BOT_TOKEN --project-name tg-dualbot-cloudflare` |
| `PANEL_USER` | Secret，可选 | 不设置默认 `admin` |
| `PANEL_PASSWORD` | Secret | 后台登录密码 |
| `PANEL_SECRET` | Secret | Cookie Session 签名 |
| `TELEGRAM_SECRET_TOKEN` | Secret | Telegram Webhook 校验 |
| `TURNSTILE_SECRET_KEY` | Secret | Turnstile 服务端校验 |

Pages 模式下，`wrangler.toml` 不会自动替你配置 Pages 项目的 D1 binding 和环境变量，必须在 Pages 项目设置中单独配置。

### 9D-3. Pages + GitHub 部署后检查

部署完成后会得到 Pages 地址，例如：
...
```text
https://tg-dualbot-cloudflare.pages.dev/health
```

返回 `ok` 后，把 Pages 项目的 `PUBLIC_BASE_URL` 设置为：

```text
https://tg-dualbot-cloudflare.pages.dev
```

然后重新部署 Pages，让 `/verify` 链接生成使用正确域名。

Telegram Webhook 应设置为：
...
## 第 9E 步：Pages Wrangler Direct Upload

适合不想连接 GitHub，但可以在本地运行 Wrangler 的用户。

注意：当前项目使用 `functions/[[path]].js` 作为 Pages Functions 入口。Cloudflare Dashboard 的 Pages 拖拽上传主要适合静态站点，不适合这个项目的 `functions/` 入口。这个项目的 Pages 手动部署建议使用 Wrangler Direct Upload，或者使用 `第 9D 步` 的 Pages + GitHub。
...
`DB` D1 binding、`PUBLIC_BASE_URL`、`TURNSTILE_SITE_KEY` 仍然要在 Pages 项目 Settings 中配置。

### 9E-3. 执行 Direct Upload
